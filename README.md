...
Для проверки доступности приложения и подключения к базе данных используется:

GET /api/health.php

При успешной проверке API возвращает статус `ok`, состояние подключения к базе данных и текущее время проверки.